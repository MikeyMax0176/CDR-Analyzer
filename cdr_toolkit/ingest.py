...
def validate_cdr(df: pd.DataFrame) -> dict:
    ...
    if 'imei' in df.columns:
        ...
        if len(imei_series) > 0:
            ...
            if n_invalid_imei > 0:
                warnings.append(f"{n_invalid_imei} IMEI values are not in standard 15-digit format.")
            
            # Check for obviously invalid IMEIs (all zeros, all same digit)
            all_zeros = (imei_series == '000000000000000').sum()
            all_same = imei_series.apply(lambda x: len(set(x)) == 1 if len(x) == 15 else False).sum()
            if all_zeros > 0:
                warnings.append(f"{all_zeros} IMEI values are all zeros (likely invalid).")
            if all_same > 0:
                warnings.append(f"{all_same} IMEI values have all identical digits (likely invalid).")
    ...
    if 'imsi' in df.columns:
        ...
        if len(imsi_series) > 0:
            ...
            if n_invalid_imsi > 0:
                warnings.append(f"{n_invalid_imsi} IMSI values are not in standard 14-15 digit format.")
