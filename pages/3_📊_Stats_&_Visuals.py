...
if not pairs_df.empty and {'caller', 'callee', 'count'}.issubset(pairs_df.columns):
    ...
    with tab_bar:
        ...
        bar_df = undirected.groupby('pair', as_index=False)['count'].sum().sort_values('count', ascending=False).head(20)
    ...
    with tab_sankey:
        try:
            ...
            nodes = pd.Index(sankey_df['caller']).append(pd.Index(sankey_df['callee'])).unique().tolist()
            node_idx = {n: i for i, n in enumerate(nodes)}

            st.markdown("**Sankey Color & Contrast Options**")
            # Contrast sliders (0=black, 255=white)
            node_contrast = st.slider("Node color contrast (0=black, 255=white)", 0, 255, 176, key="sankey_node_contrast")
            link_contrast = st.slider("Link color contrast (0=black, 255=white)", 0, 255, 144, key="sankey_link_contrast")
    ...
    # Directional split
    with tab_split:
        outbound = pairs_df.groupby('caller', as_index=False)['count'].sum().sort_values('count', ascending=False).head(15)
        inbound = pairs_df.groupby('callee', as_index=False)['count'].sum().sort_values('count', ascending=False).head(15)
    ...
    with tab_drill:
        ...
        if a and b:
            drill = df[((df['caller'].astype(str) == a) & (df['callee'].astype(str) == b)) |
...
if target:
    ...
    st.download_button("Download Top Contacts CSV", contacts_df.to_csv(index=False), "top_contacts.csv")
