...
if os.path.exists(network_file):
    try:
        ...
        # Download button
        st.download_button("📥 Download Network HTML", html_content, network_file, mime="text/html")

    except Exception as e:
        st.error(f"Error loading network file: {e}")
else:
    st.warning(
        "⚠️ No network file found. Please generate a network first in the Network Explorer page."
    )
