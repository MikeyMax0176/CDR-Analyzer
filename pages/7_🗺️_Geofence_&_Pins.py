...
with st.sidebar:
        ...
        with st.form("quick_case"):
            ...
            if st.form_submit_button("Create"):
                if quick_case_name:
                    try:
                        case_id = ensure_case(
                            st.session_state.engine, quick_case_name, quick_case_desc
                        )
    ...
    if create_event_on_click:
        ...
        event_title_template = st.text_input(
            "Event Title Template", value="Location Event {counter}"
        )
...
if current_case_id:
    ...
    with tab3:
        if location_events:
            ...
            st.info(
                "No location events for this case. Add events with coordinates in the Cases page or enable 'Create event on click'."
            )
