        ...
        with st.expander("➕ Add New Note", expanded=False):
            with st.form("add_note"):
                note_title = st.text_input(
                    "Note Title", placeholder="Meeting notes, observation, etc."
                )
                note_content = st.text_area(
                    "Content", placeholder="Detailed note content...", height=150
                )
                ...
                if st.form_submit_button("Save Note"):
                    if note_title and note_content:
                        try:
                            note_id = add_note(
                                st.session_state.engine, current_case_id, note_title, note_content
                            )
        ...
        with st.expander("➕ Add New Event", expanded=False):
            with st.form("add_event"):
                ...
                with col2:
                    ...
                    phone_numbers_text = st.text_input(
                        "Phone Numbers (comma-separated)", placeholder="123456789, 987654321"
                    )

                event_description = st.text_area(
                    "Description", placeholder="Detailed event description...", height=100
                )
                tags_text = st.text_input(
                    "Tags (comma-separated)", placeholder="urgent, evidence, follow-up"
                )
                ...
                if st.form_submit_button("Save Event"):
                    if event_title:
                        try:
                            ...
                            phone_numbers = (
                                [p.strip() for p in phone_numbers_text.split(",") if p.strip()]
    ...
    with tab3:
        ...
        if events:
            ...
            for event in recent_events:
                st.markdown(
                    f"- **{event['event_type']}**: {event['title']} *({event['event_time'] or event['created_at']})*"
                )
